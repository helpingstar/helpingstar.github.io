...
확률의 연쇄법칙을 적용해 $p_\theta(\tau)$를 전개하면 다음과 같다.
...
&= p(\mathbf{s}_1)p_\theta(\mathbf{a}_1|\mathbf{s}_1)p_\theta(\mathbf{s}_2, \mathbf{a}_2, \ldots, \mathbf{s}_T, \mathbf{a}_T|\mathbf{s}_1, \mathbf{a}_1) \\
&= p(\mathbf{s}_1)p_\theta(\mathbf{a}_1|\mathbf{s}_1)p(\mathbf{s}_2|\mathbf{s}_1, \mathbf{a}_1) p_\theta(\mathbf{a}_2, \mathbf{s}_3, \ldots, \mathbf{s}_T, \mathbf{a}_T|\mathbf{s}_1, \mathbf{a}_1, \mathbf{s}_2)
\end{align*}$$

* $p(\mathbf{s}_2|\mathbf{s}_1, \mathbf{a}_1)$ : 환경 모델로 정책과 무관하기에 아래 첨자 없이 표기

마르코프 시퀀스 가정에 의해 아래를 만족한다.

$$\begin{align*}
& p_\theta(\mathbf{a}_2 | \mathbf{s}_1, \mathbf{a}_1, \mathbf{s}_2) = \pi_\theta(\mathbf{a}_2 | \mathbf{s}_2) \\
& p(\mathbf{s}_3 | \mathbf{s}_1, \mathbf{a}_1, \mathbf{s}_2, \mathbf{a}_2) = p(\mathbf{s}_3 | \mathbf{s}_2, \mathbf{a}_2)
\end{align*}$$
...
우리의 목적은 목적함수 $J(\theta)=E_{\tau \sim p_{\theta}(\tau)} \left[ \sum_t r(\mathbf{s}_t, \mathbf{a}_t) \right]$를 최대화하는 정책 파라미터 $\theta$를 계산하는 것이다.
...
기댓값은 실제로 계산할 수 없으므로 샘플을 이용해 추정한다. 기댓값 $\mathbb{E}_{\tau \sim p_\theta(\tau)}[\cdot]$은 에피소드를 $\pi_\theta$ 정책을 이용해 $N$개만큼 생성해 에피소드 평균을 이용해서 근사적으로 계산한다.
...
우리의 목적은 목적함수 $J(\theta)=E_{\tau \sim p_{\theta}(\tau)} \left[ \sum_t r(\mathbf{s}_t, \mathbf{a}_t) \right]$를 최대화하는 정책 파라미터 $\theta$를 계산하는 것이었다.
...
에서 $\nabla_\theta p_\theta(\tau)$를 위 수식에 대입하면
...
와 같이 표현된다. 이때 $\nabla_\theta \log p_{\theta}(\tau)$를 자세히 살펴보자 우리는 아래와 같이 $p_{\theta}(\tau)$를 표현했다.

$$p_\theta(\tau) = p_\theta(\mathbf{s}_1, \mathbf{a}_1, \ldots, \mathbf{s}_T, \mathbf{a}_T) = p(\mathbf{s}_1) \prod_{t=1}^T \pi_\theta(\mathbf{a}_t | \mathbf{s}_t) p(\mathbf{s}_{t+1} | \mathbf{s}_t, \mathbf{a}_t)$$

$\log p_{\theta}(\tau)$를 구하기 위해 위 식에 $\log$를 씌우면 식이 아래와 같아진다.
...
마지막으로 최종 목표인 $\nabla_\theta \log p_{\theta}(\tau)$를 구하기 위해 $\nabla_\theta$를 씌우면
...
인데 가운데 항만 $\theta$의 함수이므로 $\log p(\mathbf{s}_1)$, $\log p(\mathbf{s}_{t+1}|\mathbf{s}_t, \mathbf{a}_t)$를 제거하면
...
이제 구한 식을 원래 목표였던 $\nabla_{\theta} J(\theta)$ 식에 대입하자. 그리고 편의를 위해 잠시 바꿨던 $r(\tau)$도 돌려놓자

$$\begin{align*}
\nabla_{\theta} J(\theta) &= \int \nabla_{\theta} p_{\theta}(\tau) r(\tau) d\tau = \int p_{\theta}(\tau) \left(\nabla_{\theta} \log p_{\theta}(\tau) \right) r(\tau) d\tau \\
&= \int p_{\theta}(\tau) \left( \sum_{t=1}^{T} \nabla_\theta \log \pi_{\theta}(\mathbf{a}_t|\mathbf{s}_t) \right) r(\tau) d\tau \\
&= \int p_{\theta}(\tau) \left( \sum_{t=1}^{T} \nabla_\theta \log \pi_{\theta}(\mathbf{a}_t|\mathbf{s}_t) \right) \left( \sum_{t=1}^T r(\mathbf{s}_t, \mathbf{a}_t) \right) d\tau \\
...
$\left( \sum_{t=1}^T r(\mathbf{s}_t, \mathbf{a}_t) \right)$ 은 $(t=1)$부터 에피소드가 종료$(t=T)$될 때까지 받을 수 있는 전체 보상의 합이다. 하지만 시간 $t'$에서의 정책은 시간 $t<t'$ 인 시간 $t$에서의 보상에 영향을 끼치지 않는다. 이러한 인과성(casuality)를 고려하여 위 식을 아래와 같이 수정할 수 있다.

$$\nabla_{\theta} J(\theta) \approx \frac{1}{N} \sum_{i=1}^N \sum_{t=1}^{T} \nabla_\theta \log \pi_{\theta}(\mathbf{a}_{i,t}|\mathbf{s}_{i,t}) \left( \sum_{t'=t}^T r(\mathbf{s}_{i,t'}, \mathbf{a}_{i,t'}) \right)$$

오른쪽 식 $\left( \sum_{t'=t}^T r(\mathbf{s}_{i,t'}, \mathbf{a}_{i,t'}) \right)$은  reward-to-go 라고 하며 지금부터 에피소드 종료까지 얻을 보상을 의미한다.
...
식이 바뀌었는데 아래식이랑 같은 의미이다. 간단히 다시 말하면 위 식에서 $p_{\theta}(\tau)$에 로그를 씌워서 순차적으로 곱해져있던 trajectory를 더하기로 바꾸고 미분으로 환경모델 확률식을 제거하여 아래와 같은 식이 나온다. 전개의 편의를 위해 다시 $r(\tau)=\sum_{t=1}^T r(\mathbf{s}_{t}, \mathbf{a}_{t})$를 적용한다. 
...
이번엔 거꾸로 $p_{\theta}(\tau) \nabla_{\theta} \log p_{\theta}(\tau)$ 에 $\nabla_\theta p_\theta(\tau)$를 대입하면 아래와 같은 식이 나온다.
