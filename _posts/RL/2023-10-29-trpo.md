...
* $A\_{\pi}(s\_t, a\_t) = Q\_{\pi}(s\_t, a\_t) - V\_{\pi}(s\_t) = r(s\_t) + \gamma \mathbb{E}\_{s\_{t+1} \sim P(s\_{t+1} \vert s\_t,a\_t)}[V\_{\pi}(s\_{t+1})] - V\_{\pi}(s\_t).$
  * $\mathbb{E}\_{\tau \sim \tilde{\pi}}$는 $\mathbb{E}\_{s\_{t+1} \sim P(s\_{t+1} \vert s\_t,a\_t)}$를 포함
...
* $E\_{\tau \sim \tilde{\pi}} \left[ \sum\_{t=0}^{\infty} \gamma^t A\_{\pi}(s\_t, a\_t) \right]$
...
* $\sum\_{t=0}^{\infty} \sum\_s P(s\_t = s \vert \tilde{\pi}) \sum\_a \tilde{\pi}(a \vert s) \gamma^t A\_{\pi}(s, a)$
...
$\rho\_{\pi}$를 적용함으로써 policy update에 의한 state visitation frequency의 변화를 고려하지 않아도 된다.
...
그러므로 $\sum\_s \rho\_{\pi\_{\theta\_0}} \sum\_a \pi\_{\theta\_0}(a \vert s)A\_{\pi\_{\theta\_0}}(s,a) = 0$

* $\theta$는 old policy $\pi\_\theta$에 대한 parameter이다.
* $L\_\pi(\tilde{\pi}) = \eta(\pi) + \sum\_s \rho\_\pi(s) \sum\_a \tilde{\pi}(a \vert s)A\_\pi(s,a)$
  * $\pi \leftarrow \pi\_{\theta\_0}$
  * $L\_\pi(\pi\_{\theta\_0}) = \eta(\pi\_{\theta\_0}) + \sum\_s \rho\_{\pi\_{\theta\_0}}(s) \sum\_a \pi\_{\theta\_0}(a \vert s)A\_{\pi\_{\theta\_0}}(s,a)$
