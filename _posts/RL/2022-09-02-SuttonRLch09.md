...
전에는 상태함수가 표 형태로 표현되었으나 이제는 가중치 벡터 $\bold{w}\in\mathbb{R}^{d}$ 파라미터화된 함수 형태를 갖는다

$\hat{v}(s, \bold{w}) \approx v_{\pi}(s)$ : 가중치

$\bold{w}$가 굵은 글씨인 이유는 단일 숫자가 아닌 여러 숫자로 이루어진 벡터이기 때문인 듯 하다.

일반적으로, $\hat{v}$는 다층 인공 신경망에 의해 계산된 함수이고, $\bold{w}$는 모든 층의 연결된 가중치 벡터일 수도 있다.

일반적으로 가중치의 개수($\bold{w}$의 차원)은 상태의 개수보다 훨씬 적다($d \ll \mid \mathcal{S} \mid$), 결과적으로 하나의 상태가 갱신되면 그 변화는 다른 상태의 가치에 영향을 준다. 이러한 **일반화**(generalization)가 학습을 강력하게도, 이해가 어렵게도 만든다.
...
* `TD(0)` : $S_t \mapsto R_{t+1}+\gamma \hat{v}(S_{t+1}, \bold{w}_t)$
* `n-step TD` : $S_t \mapsto G_{t:t+n}$
* `DP policy-evaluation` : $s \mapsto \mathbb{E}_{\pi}[R_{t+1}+\gamma \hat{v}(S_{t+1}, \bold{w}_t) \mid S_t=s]$
...
$\overline{VE}(\bold{w}) \doteq \sum_{s \in \mathcal{S}} \mu(s) [v_{\pi}-\hat{v}(s,\bold{w})]^2$
...
$\bold{w}_{t+1} \doteq \bold{w}_t - \frac{1}{2} \alpha \nabla [v_{\pi}(S_t) - \hat{v}(S_t, \bold{w}_t)]^2$

$=\bold{w}_t - \alpha[v_{\pi}(S_t) - \hat{v}(S_t, \bold{w}_t)]\nabla \hat{v}(S_t,\bold{w}_t)$

* $\alpha$ : 양의 시간 간격 파라미터

$\nabla f(\bold{w}) \doteq (\frac{\partial f(\bold{w})}{\partial w_1},\frac{\partial f(\bold{w})}{\partial w_2},\cdots,\frac{\partial f(\bold{w})}{\partial w_d})^T$

$\nabla f(\bold{w})$ : 함수 $f(\bold{w})$를 벡터의 성분에 대해 편미분한 열 벡터
...
$\bold{w}_{t+1}=\bold{w}_t - \alpha[U_t - \hat{v}(S_t, \bold{w}_t)]\nabla \hat{v}(S_t,\bold{w}_t)$
...
`n-step returns` $G_{t:t+n}$이나 `DP target` $\sum_{a,s',r} \pi(a \mid S_t)p(s',r \mid S_t, a)[r + \gamma \hat{v}(s', \bold{w}_t)]$ 는 모두 가중치 벡터 $\bold{w}_t$의 영향을 받는데, 이것은 부트스트랩 목표가 편차를 갖게 되어 진정한 경사도 강하 방법을 형성하지 못할것을 암시함
...
의 핵심 단계가 $\bold{w}_t$와는 독립적으로 존재하는 목표에 영향을 받는다고 보는 것이다

($\bold{w}_t$가 바뀌면 bootstrapping에서 미래의 예측치인 target도 변하게 되는데 이는 현재 예측치와 독립적이지 않다.)

부트스트랩 방법은 가중치 벡터 $\bold{w}_t$를 변경하는 것이 추정값에 미치는 효과를 고려하지만 target에 미치는 효과는 무시한다. 이 방법은 오직 경사도의 일부만을 포함하고 따라서 이 방법을 **준경사도 방법**(semi-gradient method)이라고 부른다.
