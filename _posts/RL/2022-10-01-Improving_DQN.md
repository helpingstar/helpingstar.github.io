...
훈련 도중에 $\hat{Q^\pi}(s,a)=Q^{\pi_\theta}(s,a)$와 $Q^{\pi}_\text{tar}$의 차이를 최소화하기 위해 $Q$ 네트워크 파라미털 $\theta$를 조정하는데, $Q^{\pi}_\text{tar}$의 값이 훈련 단계마다 변하는 경우 이러한 조정이 어려워진다.
...
$\varphi$는 $\theta$의 현재 값으로 주기적으로 업데이트된다. 이것을 치환 업데이트(replacement update)라고 부른다. $\varphi$의 업데이트 주기는 문제마다 다르며 간단한 문제일 수록 업데이트 주기가 작아도 충분하다. $Q^{\pi_\theta}_{\text{tar}}(s,a)$가 계산될 때마다, 파라미터 $\theta$로 표현되는 $Q$ 함수는 조금 달라질 것이기 때문에 $Q^{\pi_\theta}_{\text{tar}}(s,a)$는 동일한 $(s,a)$에 대해 다른 값을 갖게 될 것이다. 이러한 '움직이는 목표'는 네트워크가 어떤 값을 도출해야 하는지를 모호하게 만들기 떄문에 훈련이 불안정해진다.
