...
볼츠만 정책을 생성하기 위해 `softmax` 함수를 적용함으로써 상태 $s$에서 모든 행동 $a$에 대한 $Q$ 가치의 확률분포를 만든다. 소프트맥스 함수에 온도(temperature) 파라미터 $\tau \in (0, \infin)$를 추가하여 확률 분포의 균일한 정도나 집중된 정도가 얼마인지를 조정할 수 있다.
...
$Q^*(s,a_1)=2,Q^*(s,a_2)=10$이라 하고 현재 추정값이 $\hat{Q}^\pi(s,a_1)=2.5,\hat{Q}^\pi(s,a_2)=-3$이라고 하자 이러면 $\tau=1$일 경우 $p(x)=[0.9959..., 0.0040...]$이다. 이러면 $a_2$를 탐험할 확률은 거의 사라질 것이다. 하지만 `ε-greedy` 정책은 $Q$ 가치 추정값에 관계없이 $p=\varepsilon/2$의 확률로 $a_2$를 선택한다. 이러면 시간이 지남에 따라 $Q$ 가치 추정값이 보정될 가능성이 높다.
